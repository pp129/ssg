app.controller('boxDemol', ['$scope', 'toaster', '$modal',
    function ($scope, toaster, $modal) {
        ...
        $scope.deleteList = function () {
            ...
            if (deleteIDs.length <= 0) {
                ...
                modalInstance.result.then(function () { // 确定后操作
                    $.ajax({
                        url: '../common/contype/delete',
                        type: 'POST',
                        async: false,
                        data: {contypes: deleteIDs},
        ...
        function rowEdit(row) {
            ...
            $.ajax({
                url: '../common/contype/edit',
                type: 'GET',
                async: false,
                data: {contype: row.contype},
                success: function (data) {
                	console.log(data)
...
app.controller('funModalInstanceCtrl', ['$scope', '$modalInstance', 'items',
    function ($scope, $modalInstance, items) {
        ...
        $scope.formData = {};
        if (items.type === 'add') {
            ajaxUrl = '../common/contype/save';
        } else if (items.type === 'update') {
            ajaxUrl = '../common/contype/update';
