app.controller('funInfo', ['$scope', 'toaster', '$modal',
    function ($scope, toaster, $modal) {
        ...
        $scope.deleteList = function () {
            ...
            if (deleteIDs.length <= 0) {
                ...
                modalInstance.result.then(function () { // 确定后操作
                    $.ajax({
                        url: '../common/consize/delete',
                        type: 'POST',
                        async: false,
                        data: {consizes: deleteIDs},
        ...
        function rowEdit(row) {
            ...
            $.ajax({
                url: '../common/consize/edit',
                type: 'GET',
                async: false,
                data: {consize: row.consize},
                success: function (data) {
...
app.controller('funModalInstanceCtrl', ['$scope', '$modalInstance', 'items','toaster',
    function ($scope, $modalInstance, items,toaster) {
        ...
        $scope.formData = {};
        if (items.type === 'add') {
            ajaxUrl = '../common/consize/save';
        } else if (items.type === 'update') {
            ajaxUrl = '../common/consize/update';
