...
app.controller('test', ['$scope', 'toaster', '$modal', '$http', '$stateParams', '$state', '$rootScope', 'commonTools',
...
app.controller('userModalInstanceCtrl', ['$scope', '$modalInstance', 'items', 'toaster', 'CommonQuery', 'commonTools', '$modal',
    function ($scope, $modalInstance, items, toaster, CommonQuery, commonTools, $modal) {
        ...
        $scope.check = function (value, roleid, checked) {
            //console.log(value)
            //delete value.$$hashKey;
            var idx = roleidArr.indexOf(roleid);
            if (idx >= 0 && !checked) {
                for (var i = 0; i < $scope.formData.roles.length; i++) {
                    if ($scope.formData.roles[i].roleid === roleid) {
                        $scope.formData.roles.splice(i, 1);
                        break;
                    }
                }
                roleidArr.splice(idx, 1);
            }
            if (idx < 0 && checked) {
                $scope.formData.roles.push(value);
                roleidArr.push(roleid);
            }
        };
        $scope.getChecked = function () {
            return $scope.formData.roles
        };
...
app.controller('rolesModalInstanceCtrl', function ($scope, $modalInstance, items) {
