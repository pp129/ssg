...
(function(g) {
	...
	v(y.prototype, {
		renderMapNavigation: function() {
			...
			if (b.enableButtons)
				for (d in c)
					if (c.hasOwnProperty(d)) f = p(b.buttonOptions, c[d]), e = a.renderer.button(f.text, 0, 0, i).attr({
						width: f.width,
						height: f.height
	...
	k.map = g.extendClass(k.scatter, {
		...
		init: function(a) {
			...
			else if (j) f = j.from, i = j.to, h = j.fromLabel, j = j.toLabel, m = o ? [0, 0, 1, 0] : [0, 1, 0, 0], o || (o = h, h = j, j = o), o = {
				linearGradient: {
					x1: m[0],
					y1: m[1],
					x2: m[2],
